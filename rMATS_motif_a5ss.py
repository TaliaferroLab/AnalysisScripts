...
def geteventstofilter(rMATStable):
	...
	for index, row in df.iterrows():
		chrm = row['chr']
		strand = row['strand']
		longexonstart = row['longExonStart_0base']
		longexonend = row['longExonEnd']
		shortexonstart = row['shortES']
		shortexonend = row['shortEE']
		downstreamexonstart = row['flankingES']
		downstreamexonend = row['flankingEE']
		eventID = chrm + ':' + strand + ':' + str(longexonstart) + '-' + str(longexonend) + ':' + str(shortexonstart) + '-' + str(shortexonend) + ':' + str(downstreamexonstart) + '-' + str(downstreamexonend)
		FDR = row['FDR']
...
def getSequences(rMATStable, genomefasta, sensevents):
	...
	for index, row in df.iterrows():
		...
		if strand == '+':
			stretch1 = min(150, shortexonend - shortexonstart)
			stretch2 = min(150, longexonend - shortexonend)
			stretch3 = min(150, longexonend - shortexonend)
			stretch4 = min(150, downstreamexonstart - longexonend)

		elif strand == '-':
			stretch1 = min(150, shortexonend - shortexonstart)
			stretch2 = min(150, shortexonstart - longexonstart)
			stretch3 = min(150, shortexonstart - longexonstart)
			stretch4 = min(150, longexonstart - downstreamexonend)


		if strand == '+':
			seq1 = seq_dict[chrm].seq[shortexonend - stretch1 : shortexonend]
			seq2 = seq_dict[chrm].seq[shortexonend : shortexonend + stretch2]
			seq3 = seq_dict[chrm].seq[longexonend - stretch3 : longexonend]
			seq4 = seq_dict[chrm].seq[longexonend : longexonend + stretch4]

		elif strand == '-':
			seq1 = seq_dict[chrm].seq[shortexonstart : shortexonstart + stretch1].reverse_complement()
			seq2 = seq_dict[chrm].seq[shortexonstart - stretch2 : shortexonstart].reverse_complement()
			seq3 = seq_dict[chrm].seq[longexonstart : longexonstart + stretch3].reverse_complement()
			seq4 = seq_dict[chrm].seq[longexonstart - stretch4 : longexonstart].reverse_complement()
