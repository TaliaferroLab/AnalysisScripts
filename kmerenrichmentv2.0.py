...
import argparse
from kmertoprotein_v2 import getPSSMs, makeallkmers, motiftoprotein, getpassingkmers, getpassingproteins
...
def countKmers(fasta1, fasta2, k):
    ...
    for seq_record in SeqIO.parse(fasta1, 'fasta'):
        seq = str(seq_record.seq.transcribe().upper())
    ...
    p_adjust = stats.p_adjust(FloatVector(pvalues), method = 'BY')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--matchthreshhold', type = float, help = 'To be called a match for a protein, a kmer must be in the top [threshhold] percentile in both RNAcompete biological replicates. Value must be between 0 and 1.')
    ...
    if args.kmertoprotein == False:
        outfh.write('kmer' + '\t' + str(args.sequences) + '_count' + '\t' + str(args.background) + '_count' + '\t' 'enrichment' + '\t' + 'fishers_exact_p' + '\t' + 'BH_corrected_p''\n')
        ...
        outfh.write('kmer' + '\t' + str(args.sequences) + '_count' + '\t' + str(args.background) + '_count' + '\t' 'enrichment' + '\t' + 'fishers_exact_p' + '\t' + 'BH_corrected_p' + '\t' + 'Best_protein_matches''\n')
        motiftoproteindict = motiftoprotein(args.masterfile)
        PSSMs = getPSSMs(args.pssmdirectory)
        allkmers = makeallkmers(args.k)
        passingkmers = getpassingkmers(PSSMs, allkmers, args.matchthreshhold, motiftoproteindict) #{protein : [kmers that pass filters as matches]}
        passingproteins = getpassingproteins(passingkmers, allkmers)
        
        for kmer in countKmers(args.sequences, args.background, args.k):
            kmerseq = kmer[0]
            proteinmatcheslist = passingproteins[kmerseq] #this is a list
            proteinmatches = (', ').join(proteinmatcheslist)

            outfh.write(('\t').join([str(item) for item in kmer]) + '\t' + proteinmatches + '\n')
