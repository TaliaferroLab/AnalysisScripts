...
def getSequences(rMATStable, genomefasta, sensevents):
	...
	if os.path.basename(genomefasta).endswith('.gz'):
		...
		seq_dict = SeqIO.to_dict(SeqIO.parse(open(genomefasta), 'fasta'))
	print('Done indexing!')
...
def searchfasta(fasta, windowsize, slidesize, motifs):
	seqsinfile = 0
	...
	countermotifdict = {} #{0-based window center : [number of nt covered by motif in this window across all seqs, number of nt in this window across all seqs]} #This is a summary dict.

	#Populate countergmotifdict with 0s
	for i in range(200):
		countermotifdict[i] = [0, 0]
	...
	for record in SeqIO.parse(fasta, 'fasta'):
		...
		while windowstart + windowsize <= len(seq):
			...
			if windowcenter in countermotifdict:
				countermotifdict[windowcenter][0] += len(coverednt)
				countermotifdict[windowcenter][1] += len(windowseq)
			elif windowcenter not in countermotifdict:
				countermotifdict[windowcenter] = [len(coverednt), len(windowseq)]
...
def iteratefastas(motifs):
	#Motifs is a list of many motifs (if scramblecontrol) or a list of just one motif (if no scramblecontrol)
	...
	for fasta in fastas:
		...
		fastamotifmatchdict = searchfasta(fasta, 40, 1, motifs) #{seqname : {0-based window center : fraction of nt covered by motif in this window}}
		#Add to DataFrame
...
def plotbigdf():
	...
	#Save
	p.save(filename = './means.f05d05.pdf')
	...
	statdf = pd.concat([statdf, df, df2], axis = 1)
	statdf.to_csv('statdf.txt', sep = '\t', index = False)


	#plot
	#p = ggplot(statdf, aes(x = 'pos', y = 'np.log10(pval) * -1', color = 'sample')) + geom_line() + facet_wrap('region')
	p = (ggplot(statdf, aes(x = 'pos', y = 'np.log10(pval) * -1', color = 'sample')) + geom_line() + facet_wrap('region'))
	p.save(filename = './pvalues.f05d05.pdf')
