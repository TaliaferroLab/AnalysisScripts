...
def longestORF(gff, genes_of_interest_file, sequencedir):
    ...
    sys.stderr.write('%i sequences indexed \n' % len(seqdb))
        ...
        if gene.id in genes_of_interest:
            ...
            if gene.strand == '+':
                for exon in longestORFs[gene.id]:
                    ...
                    exonstop = int(exon[1])
                    sequence += seqdb[gene.chrom].seq[exonstart-1:exonstop]

            elif gene.strand == '-':
                for exon in reversed(longestORFs[gene.id]): #reverse exon order
                    exonstart = int(exon[0])
                    exonstop = int(exon[1])
                    sequence += seqdb[gene.chrom].seq[exonstart-1:exonstop].reverse_complement()
...
if __name__ == '__main__':
    ...
    for ID in seqdictionary:
        outfh.write('>' + ID + '\n' + str(seqdictionary[ID].translate()) + '\n')
