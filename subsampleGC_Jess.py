...
def subsampleGC(modelfasta, subsamplefasta):
    ...
    for record in SeqIO.parse(modelfasta, 'fasta'):
        ...
        modelfastarecords +=1
        #Append length to modellengths
        modellengths.append(len(record.seq))
    ...
    shortestmodellength = min(modellengths)

    print 'Shortest length in model is {0}'.format(shortestmodellength)


    subsamplefasta = 'temp.fasta'
    #Populate subsamplebins
    for record in SeqIO.parse(subsamplefasta, 'fasta'):
        if len(record.seq) >= shortestmodellength:
            passedlengthfilter +=1
            lowerbound = 20
            upperbound = 22
            GCcontent = float(GC(record.seq))
    ...
    print '{0} sequences in the file to be subsampled passed the length filter'.format(passedlengthfilter)
