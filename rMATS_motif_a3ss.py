...
def geteventstofilter(rMATStable):
	...
	for index, row in df.iterrows():
		...
		upstreamexonstart = row['flankingES']
		upstreamexonend = row['flankingEE']
		eventID = chrm + ':' + strand + ':' + str(longexonstart) + '-' + str(longexonend) + ':' + str(shortexonstart) + '-' + str(shortexonend) + ':' + str(upstreamexonstart) + '-' + str(upstreamexonend)
		FDR = row['FDR']
...
def getSequences(rMATStable, genomefasta, sensevents):
	...
	for index, row in df.iterrows():
		...
		if strand == '+':
			stretch1 = min(150, longexonstart - upstreamexonend)
			stretch2 = min(150, shortexonstart - longexonstart)
			stretch3 = min(150, shortexonstart - longexonstart)
			stretch4 = min(150, shortexonend - shortexonstart)

		elif strand == '-':
			stretch1 = min(150, upstreamexonstart - longexonend)
		...
		if strand == '+':
			seq1 = seq_dict[chrm].seq[longexonstart - stretch1 : longexonstart]
			seq2 = seq_dict[chrm].seq[longexonstart : longexonstart + stretch2]
			seq3 = seq_dict[chrm].seq[shortexonstart - stretch3 : shortexonstart]
			seq4 = seq_dict[chrm].seq[shortexonstart : shortexonstart + stretch4]

		elif strand == '-':
			seq1 = seq_dict[chrm].seq[longexonend : longexonend + stretch1].reverse_complement()
			seq2 = seq_dict[chrm].seq[longexonend - stretch2 : longexonend].reverse_complement()
			seq3 = seq_dict[chrm].seq[shortexonend : shortexonend + stretch3].reverse_complement()
			seq4 = seq_dict[chrm].seq[shortexonend - stretch4 : shortexonend].reverse_complement()
