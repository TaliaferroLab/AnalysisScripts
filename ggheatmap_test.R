...
#my.colours <- c('yellow','darkorange','orangered')
my.colours <- rev(brewer.pal(11, name = 'RdYlBu'))
...
ggheatmap <- function(x,
  ...
  #col.hc <- hclust(JSdist(makeprobs(t(x))), 'ward')
  ...
  col.plot <- mydplot(col.dendro, col=TRUE, labels=TRUE) +
    ...
    theme(plot.margin = unit(c(0,0,1,0), "lines"), axis.text.x = element_text(size = 8, hjust = 0.6, angle = 20))
  print(col.hc$labels[col.hc$order])
  #print(row.hc$labels[row.hc$order])
  ...
  centre.plot <- ggplot(xx, aes(X2,X1)) + geom_tile(aes(fill=value)) +
    scale_fill_gradientn(colours = hm.colours, limits = c(-0.1,1), name = 'Spearman rho') +
    labs(x = NULL, y = NULL) +
    scale_x_continuous(expand=c(0,0)) +
    scale_y_continuous(expand=c(0,0),breaks = NULL) +
    theme(plot.margin = unit(rep(0, 4), "lines")) +
    scale_color_hue(c=1000)
