...
def getSequences(rMATStable, genomefasta, sensevents):
	...
	for index, row in df.iterrows():
		...
		if row['FDR'] < 0.01 and row['IncLevelDifference'] >= 0.05 and (sensevents == None or (eventID not in sensevents)):
			eventclass = 'moreincluded'
		elif row['FDR'] < 0.01 and row['IncLevelDifference'] <= -0.05 and (sensevents == None or (eventID not in sensevents)):
...
def plotbigdf():
	...
	#Save
	p.save(filename = './means.f01d05.pdf')
	...
	p.save(filename = './pvalues.f01d05.pdf')
