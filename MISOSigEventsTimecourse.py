...
def MISOcompare(MISOdirectory): 
    ...
    eventtypes = ['AFE', 'ALE', 'MXE', 'RI', 'SE', 'TandemUTR', 'SE_NMDEXC', 'SE_NMDINC'] #change this according to event type you are interested in
            ...
            for line in bffh:
                ...
                if numberofisoforms == 2:
                    ...
                    if abs(deltapsi) >= 0.1 and bayesfactor >= 10 and sample1countspass and sample2countspass:
                        sigeventcounter +=1
                        sigevents[eventtype].append(eventname)
    ...
    for eventtype in sigevents:
        sigevents[eventtype] = list(set(sigevents[eventtype]))
...
if __name__ == '__main__':
    ...
    PSIs, samples = getPSIs(args.MISOdirectory, sigevents)
    outfh = open(args.outfile , 'w')
    outfh.write('Event' + '\t' + 'EventType' + '\t' + ('\t').join(sorted(samples)) + '\n')
