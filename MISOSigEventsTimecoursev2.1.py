...
#Version 2.1 will output all events that have PSI values in all samples, including those that were not signficant
#in any sample.  The "SigComparisons" field is denoted as "None".
...
#readcountfilters['ALE'] = [100, 100, 500]
readcountfilters['MXE'] = [1, 1, 10]
readcountfilters['RI'] = [1, 1, 10]
readcountfilters['SE'] = [1, 1, 10]
readcountfilters['TandemUTR'] = [10, 0, 50]
#readcountfilters['TandemUTR'] = [100, 0, 400]
#readcountfilters['aa'] = [20, 20, 50]
#readcountfilters['ab'] = [20, 20, 50]
#readcountfilters['ac'] = [20, 20, 50]
#readcountfilters['ad'] = [20, 20, 50]
#readcountfilters['ae'] = [20, 20, 50]
#readcountfilters['af'] = [20, 20, 50]
#readcountfilters['ag'] = [20, 20, 50]
#readcountfilters['ah'] = [20, 20, 50]
#readcountfilters['ai'] = [20, 20, 50]
#readcountfilters['aj'] = [20, 20, 50]
...
def MISOcompare(MISOdirectory): 
    ...
    eventtypes = ['SE'] #change this according to event type you are interested in
        ...
        for comparison in comparisons:
            ...
            for line in bffh:
                ...
                if numberofisoforms == 2:
                    ...
                    if abs(deltapsi) >= 0 and bayesfactor >= 5 and sample1countspass and sample2countspass:
                        sigeventcounter +=1
        ...
        for sampledir in sampledirs:
            ...
            for line in misosummaryfh:
                ...
                if eventname == 'event_name': #skip header
                    continue
                if eventname not in sigevents[eventtype]:
                	#Don't consider anything with more than two isoforms (i.e. more than one PSI value)
                	#This had already been filtered out for sig events but has not been done for events that were not significant in any comparison
                	if ',' in line[1]:
                		continue
                	PSI = float(line[1])
                	sigcomparisons = 'None'
    ...
    for eventtype in PSIs:
        for event in PSIs[eventtype]:
            if len(PSIs[eventtype][event]) == len(samples): #only consider events with a PSI value for every sample
                ...
                if sigcomparisondict[eventtype][event] == 'None':
                	outfh.write('\t' + sigcomparisondict[eventtype][event])
                elif sigcomparisondict[eventtype][event] != 'None':
